...
if (!amend && !hasStagedChanges) {
  console.error("No staged changes found. Stage files first with `git add`, or use `--amend` to rename HEAD.");
  process.exit(1);
}

const branch = runGit(["rev-parse", "--abbrev-ref", "HEAD"]).trim();
const changedFiles = amend && !hasStagedChanges
...
const diff = amend && !hasStagedChanges
  ? runGit(["show", "--format=fuller", "--stat", "--patch", "--find-renames", "--find-copies", "--no-ext-diff", "--unified=3", "HEAD"])
  : runGit(["diff", "--cached", "--stat", "--patch", "--find-renames", "--find-copies", "--no-ext-diff", "--unified=3"]);
...
const message = generateCommitMessage({
  ...
  currentMessage,
  diff,
  mode: amend ? "amend the current commit message" : "create a new commit message from staged changes",
});
