...
export class PhotoSwipeToolbarComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    document.addEventListener(SITE_EVENTS.photoSwipeState, this.handleState);
    document.addEventListener(SITE_EVENTS.photoSwipeShareResult, this.handleShareResult);
  }

  ngOnDestroy() {
    if (typeof document === "undefined") return;
    document.removeEventListener(SITE_EVENTS.photoSwipeState, this.handleState);
    document.removeEventListener(SITE_EVENTS.photoSwipeShareResult, this.handleShareResult);
    window.clearTimeout(this.shareLabelTimer);
    this.hideTooltip();
  ...
  act(action: PhotoSwipeAction) {
    this.hideTooltip();
    document.dispatchEvent(new CustomEvent(SITE_EVENTS.photoSwipeAction, { detail: { action } }));
  }
