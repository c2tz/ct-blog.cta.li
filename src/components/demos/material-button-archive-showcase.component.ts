...
type IconButtonKind = "icon" | "fab" | "miniFab";
...
interface IconButtonSection {
  kind: IconButtonKind;
  label: string;
  items: IconButtonItem[];
}

interface ExtendedFabItem {
  disabled?: boolean;
  href?: string;
  icon: string;
  text: string;
...
interface ExtendedFabSection {
  label: string;
  items: ExtendedFabItem[];
}
...
@Component({
  ...
  template: `
    <div class="material-button-template" aria-label="Boutons Angular Material de l'archive">
      ...
      @for (section of iconButtonSections; track section.label) {
        <mat-divider></mat-divider>
        <section class="material-button-template-section">
          <div class="example-label">{{ section.label }}</div>
          <div class="example-button-row">
      ...
      <mat-divider></mat-divider>
      <section class="material-button-template-section">
        <div class="example-label">{{ extendedFabSection.label }}</div>
        <div class="example-button-row">
          <div class="example-flex-container">
            @for (item of extendedFabSection.items; track item.text) {
              @if (item.href) {
                <a
                  matFab
                  extended
...
export class MaterialButtonArchiveShowcaseComponent {
  ...
  readonly iconButtonSections: IconButtonSection[] = [
    {
      label: "Icon",
      kind: "icon",
      items: [
  ...
  readonly extendedFabSection: ExtendedFabSection = {
    label: "Extended FAB",
    items: [
      { text: "Basic", icon: "\uE87D" },
      { text: "Disabled", icon: "\uE87D", disabled: true },
      { text: "Link", icon: "\uE87D", href: GOOGLE_LINK },
    ],
  };
}
