...
export class GlobalTooltipComponent implements AfterViewInit, OnDestroy {
  ...
  private readonly handlePointerOver = (event: PointerEvent) => {
    ...
    if (recentTouchMode === "temporary" && target) {
  ...
  ngAfterViewInit() {
    ...
    document.addEventListener(SITE_EVENTS.tooltipHide, this.handleHideRequest);
  ...
  ngOnDestroy() {
    ...
    document.removeEventListener(SITE_EVENTS.tooltipHide, this.handleHideRequest);
